CREATE OR REPLACE FUNCTION store.invoice_get_all(
   _state text
)  
 RETURNS jsonb
LANGUAGE plpgsql  
...
$$  
	...
	BEGIN  
		...
		WHERE LOWER(_state) = 'all' or ds.state = LOWER(_state);
...
CREATE OR REPLACE FUNCTION providers.invoice_get_all(
   _state text
)  
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
	...
	result := store.invoice_get_all(_state);
    RETURN result;
END
$$;
...
select store.invoice_get_all('active')
